...
ON P.PROD_CODE=B.PROD_CODE
GROUP BY P.PROD_CODE ,P.PROD_NAME;--코드와 이름이 동일한 데이터를 하나로 봄(동일한 이름이 나타날수 있다는것에 대한 안전장치로 같은 코드를 조건으로 걸어줌)
...
-- LHJ     이휘재  2         80
-- KHD     강호동  3         1210
SELECT U.USER_ID AS 아이디,U.USER_NAME AS 고객명
        ,COUNT(B.BUY_NO) AS 구매횟수
...
-- 박수홍  3
-- 강호동  3
SELECT COUNT(B.BUY_NO)AS 구매횟수,U.USER_NAME AS 고객명
FROM USER_T U INNER JOIN BUY_T B
ON U.USER_NO=B.USER_NO
...
--삭제
DELETE 
FROM PRODUCT_T WHERE PROD_CODE=1;
COMMIT;
--삭제 후 구매내역
SELECT *FROM BUY_T;
