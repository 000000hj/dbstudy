...
(
    ...
    ,MEM_ID      VARCHAR2(30 BYTE) NOT NULL UNIQUE--회원아이디
    ,MEM_PW      VARCHAR2(30 BYTE) NOT NULL       --회원패스워드
    ,MEM_POINT   NUMBER                           --회원포인트
    ,MEM_GUBUN   VARCHAR2(7 BYTE)                 --회원구분
    ,MEM_EMAIL   VARCHAR2(100 BYTE)         UNIQUE--회원이메일
    ,CONSTRAINT MEM_GUBUN CHECK(MEM_GUBUN IN('REGULAR','SOCIAL'))
);


-- 2. MEMBER_T 테이블에 다음 새로운 칼럼을 추가하시오.
--    1) 회원주소: ADDRESS VARCHAR2(200 BYTE)
ALTER TABLE MEMBER_T    ADD ADDRESS     VARCHAR2(200 BYTE);--회원주소

--    2) 회원가입일: JOINED_DT DATE
ALTER TABLE MEMBER_T    ADD JOINED_DT   DATE;            --회원가입일
...
(
    ...
    ,TITLE          VARCHAR2(4000 BYTE)  NOT NULL       --글제목
    ,CONTENT        CLOB                 NOT NULL       --글내용
    ,HIT            VARCHAR2(1 BYTE)                    --조회수
    ,MEM_ID         VARCHAR2(30 BYTE)    NOT NULL       --작성자
    ,CREATED_DT     DATE                                --작성일자
)
