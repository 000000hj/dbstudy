...
SELECT  *
FROM EMPLOYEE_T
WHERE SALARY IN(SELECT MAX(SALARY) FROM EMPLOYEE_T);
...
SELECT *
FROM EMPLOYEE_T
WHERE MONTHS_BETWEEN(SYSDATE,TO_DATE(HIRE_DATE))>=(SELECT AVG(MONTHS_BETWEEN(SYSDATE,TO_DATE(HIRE_DATE)))
FROM EMPLOYEE_T);
...
WHERE SALARY>=(SELECT MAX(SALARY)
                FROM EMPLOYEE_T
                WHERE DEPART=(SELECT DEPT_NO FROM DEPARTMENT_T WHERE DEPT_NAME='영업부'));--영업부의 직원만 출력
...
SELECT EMP_NO,연봉,입사번호
FROM(SELECT ROW_NUMBER()OVER(ORDER BY HIRE_DATE )AS 입사번호,EMP_NO,SALARY AS 연봉 FROM EMPLOYEE_T)
WHERE 입사번호 BETWEEN 3 AND 4;
