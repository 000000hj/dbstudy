...
SELECT  EMPLOYEE_ID, FIRST_NAME, LAST_NAME, EMAIL, PHONE_NUMBER, HIRE_DATE,TO_CHAR(SALARY,'99,999')
...
SELECT DECODE(SUBSTR(PHONE_NUMBER,0,3),011,'MOBILE',515,'EAST',590,'WEST',603,'SOUTH',650,'NORTH')AS REGION
FROM EMPLOYEES;
...
SELECT DEPARTMENT_ID,RANK()OVER(PARTITION BY DEPARTMENT_ID ORDER BY SALARY DESC) AS 연봉순위
FROM EMPLOYEES
