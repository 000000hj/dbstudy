...
INSERT INTO BOOK_T(BOOK_ID,BOOK_NAME,PUBLISHER,PRICE)VALUES(BOOK_SEQ.NEXTVAL,'피겨 표본','굿스포츠',6000);
...
INSERT INTO CUSTOMER_T(CUST_ID, CUST_NAME,CUST_ADDR,PRICE) VALUES(CUST_SEQ.NEXTVAL,'박지성','영국','000-000-000');
INSERT INTO CUSTOMER_T(CUST_ID, CUST_NAME,CUST_ADDR,PRICE) VALUES(CUST_SEQ.NEXTVAL,'김연아','대한민국','111-111-111');
INSERT INTO CUSTOMER_T(CUST_ID, CUST_NAME,CUST_ADDR,PRICE) VALUES(CUST_SEQ.NEXTVAL,'장미란','대한민국','222-222-222');
INSERT INTO CUSTOMER_T(CUST_ID, CUST_NAME,CUST_ADDR,PRICE) VALUES(CUST_SEQ.NEXTVAL,'추신수','미국','333-333-333');
INSERT INTO CUSTOMER_T(CUST_ID, CUST_NAME,CUST_ADDR,PRICE) VALUES(CUST_SEQ.NEXTVAL,'박세리','대한민국','NULL');
COMMIT;
...
UPDATE ORDER_T SET ORDER_ID=CONCAT(TO_CHAR(ORDER_DT,'YYMMDD-'),ORDER_ID);
COMMIT;
/*
         ORDER_ID        VARCHAR2(20 BYTE)      NOT NULL,
        CUST_ID         NUMBER(11) ,             
...
--가격으로 랭크를 메겨서 1순위인것을 가져오기
SELECT *
  FROM (SELECT RANK()OVER(ORDER BY PRICE DESC) AS 순위
        ,  BOOK_ID AS 책번호
        ,BOOK_NAME AS 책이름 FROM BOOK_T)
 WHERE 순위=1;
...
-- 230709-9 10     올림픽 챔피언
  SELECT O.ORDER_ID AS 주문번호,
   ...
   WHERE ORDER_DT BETWEEN TO_DATE('20/07/05')AND TO_DATE('20/07/08')
ORDER BY ORDER_ID;
...
-- 장미란  역도 단계별 기술  20/07/10
    SELECT C.CUST_NAME,B.BOOK_NAME,O.ORDER_DT
    FROM BOOK_T B INNER JOIN ORDER_T O
    ON B.BOOK_ID = O.BOOK_ID INNER JOIN CUSTOMER_T C
    ON C.CUST_ID = O.CUST_ID
...
---'김연아'가 구매한 책이 몇 종인가?
SELECT C.CUST_NAME AS 고객명
      ,COUNT(DISTINCT O.BOOK_ID)AS 구매종류
      ,COUNT(O.ORDER_ID) AS 구매도서수
       FROM CUSTOMER_T C INNER JOIN ORDER_T O
       ON C.CUST_ID = O.CUST_ID
