/*
    ...
    3. 집합연산자
        1)UNION     : 합집합 ,중복값은 한 번만 조회
        2)UNION ALL : 합집합, 중복값도 그대로 조회
        3)INTERSECT : 교집합, 중복값은 한번만 조회
        4)MINUS     : 차집합, 첫 번째 SELECT 결과에서 두 번쩨 SELECT 결과를 뺀다.
    4. 형식
    SELECT 칼럼1, 칼럼2,...
      FROM 테이블1
      집합연산자
...
--부서 테이블에 존재하지만 사원 테이블에 존재하지 않는 부서번호 조회하기
  ...
  WITH 
        MY_SUBQUERY(N,TOTAL) AS( -- N,TOTAL 은 MY_SUBQUERY 의 칼럼을 의미한다.
        SELECT 1,1               -- N=1,TOTAL=1 을 의미하는 초기화 서브쿼리
        FROM DUAL                -- N=N+1, TOTAL = TOTAL +(N+1) 방식으로 반복해서 처리되는 부분
        UNION ALL
